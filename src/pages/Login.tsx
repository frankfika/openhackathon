...
export function Login() {
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    // For demo/mock mode, we simulate login based on the selected tab
    if (!supabaseEnabled) {
      setTimeout(() => {
        login(activeTab);
      ...
      <Card className="w-full max-w-md border-0 shadow-lg sm:rounded-xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                defaultValue={!supabaseEnabled ? `${activeTab}@openhackathon.com` : undefined}
                autoCapitalize="none"
                autoComplete="email"
                autoCorrect="off"
                disabled={isLoading}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="password"
                type="password"
                defaultValue={!supabaseEnabled ? "password" : undefined}
                disabled={isLoading}
                {...register('password')}
              />
              {errors.password && (
        ...
        <CardFooter className="flex flex-col space-y-2 text-center text-sm text-muted-foreground">
          <div>
            ...
            <Link to="/register" className="font-medium text-apple-blue hover:underline">
              Sign up
            </Link>
          </div>
        </CardFooter>
