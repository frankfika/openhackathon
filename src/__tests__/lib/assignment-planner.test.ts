...
describe('planBulkAssignments', () => {
  it('creates missing assignments for every selected judge and project', () => {
    const result = planBulkAssignments({
      projects,
      judgeIds: ['j1', 'j2'],
      existingAssignments: [{ projectId: 'p1', judgeId: 'j1' }],
    })

    expect(result.assignments).toEqual([
      { projectId: 'p1', judgeId: 'j2' },
      { projectId: 'p2', judgeId: 'j1' },
      { projectId: 'p2', judgeId: 'j2' },
      { projectId: 'p3', judgeId: 'j1' },
      { projectId: 'p3', judgeId: 'j2' },
    ])
  })
})
...
describe('planBalancedRandomAssignments', () => {
  ...
  it('balances new assignments toward the least-loaded judges', () => {
    ...
    expect(result.assignments).toEqual([
      { projectId: 'p4', judgeId: 'j2' },
      { projectId: 'p3', judgeId: 'j3' },
    ])
  })
})
